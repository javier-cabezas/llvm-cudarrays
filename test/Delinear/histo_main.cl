        ...
        __global unsigned int *global_overflow)
{
        ...
        unsigned int local_scan_load;

        local_scan_load = get_group_id(0) * blockDimx + get_local_id(0);

        // clearMemory (sub_histo);
        {
        int i, blockDimx;
        i = get_local_id(0); blockDimx = get_local_size(0);
        for ( ; i < BINS_PER_BLOCK / 4; i += blockDimx)
        {
                ((__local unsigned int*)sub_histo)[i] = 0;
        }
        }
